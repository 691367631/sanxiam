...
<div id="productDg-toolbar" class="topjui-toolbar"
   ...
   <!--    <a id="edit" href="javascript:void(0)">生成</a>
    ...
    <form id="queryForm" class="search-box" action="<%=request.getContextPath()%>/marketingdata/userDataComparisonlistReport">
...
<script>
    ...
    $(function () {
       ...
       $("#productDg").iDatagrid({
            id: 'productDg',
            url: '<%=request.getContextPath()%>/marketingdata/userDataComparisonlistJson',
            columns: [[
                {field: 'user_id', title: '用户id （本次）', sortable: true,width:'100px'},
                {field: 'num', title: '订单数（本次）', sortable: true,width:'100px'},
                {field: 'id', title: '订单id(本次)', sortable: true,width:'200px'},
                {field: 'last_user_id', title: '用户id （对比）', sortable: true,width:'100px'},
                {field: 'last_num', title: '订单数（对比）', sortable: true,width:'100px'},
                {field: 'created_at', title: '下单时间（对比）', sortable: true,width:'100px'},
                {field: 'mobile', title: '用户手机号', sortable: true,width:'100px'},
                {field: 'email', title: '用户邮箱', sortable: true,width:'120px'},
                {field: 'user_locale', title: '语言（本次）', sortable: true,width:'120px'},
                {field: 'platform', title: '来源（本次）', sortable: true,width:'120px'},
                {field: 'app_version', title: 'app版本（本次）', sortable: true,width:'120px'},
                {field: 'number', title: '订单编号（本次）', sortable: true,width:'100px'},
                {field: 'order_status', title: '订单状态（本次）', sortable: true,width:'100px'},
                {field: 'bilv', title: '增长率', sortable: true,width:'100px'},
                {field: 'detil', title: '订单地址信息（本次）', sortable: true,width:'100px'},
