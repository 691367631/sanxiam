...
<!--     <a id="export" href="javascript:void(0)">导出</a> -->
    ...
    <form id="queryForm" class="search-box">
        <input type="text" name="text" data-toggle="topjui-textbox"
               data-options="id:'text',prompt:'账号'">
        <input type="text" name="serialno" data-toggle="topjui-textbox"
               data-options="id:'serialno',prompt:'编号'">
...
<script>
    ...
    $(function () {
        ...
        $("#productDg").iDatagrid({
            id: 'productDg',
            url: '<%=request.getContextPath()%>/basicSetting/tgAdminlistJson',
            columns: [[
                {field: 'id', title: 'UUID', checkbox: true}, 
                {field: 'email', title: '账号', sortable: true},
                {field: 'tg_role_id', title: '角色id', sortable: true},
        ...
        $("#add").iMenubutton({
            ...
            dialog: {
                id: 'userAddDialog',
                height: 350,
                href:'<%=request.getContextPath()%>/basicSetting/tgAdminaddUI',
                buttonsGroup: [
                    {
                        text: '保存',
                        url:'<%=request.getContextPath()%>/basicSetting/tgAdminadd',
                        iconCls: 'fa fa-plus',
                        handler: 'ajaxForm',
        ...
        $("#edit").iMenubutton({
            ...
            dialog: {
                width: 950,
                height: 500,
                href:'<%=request.getContextPath()%>/basicSetting/tgAdmineditUI?serialno={id}',
                buttonsGroup: [
                    {
                        text: '保存',
                        url: '<%=request.getContextPath()%>/basicSetting/tgAdminedit',
                        iconCls: 'fa fa-save',
                        handler: 'ajaxForm',
                        btnCls: 'topjui-btn'
                    }
        ...
        $("#delete").iMenubutton({
            ...
            url:'<%=request.getContextPath()%>/basicSetting/tgAdmindelete'
        });
