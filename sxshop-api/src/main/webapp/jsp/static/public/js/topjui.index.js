...
function logout() {
    $.iMessager.confirm('提示', '确定要退出吗?', function (r) {
        if (r) {
            ...
            window.location.href = '/logout' + location.search;
        }
    });
}
...
function generateMenu(menuId, systemName) {
    ...
    if (menuId == "") {
        ...
        var url = "/index/getTwoMenu?id="+menuId;
        $.get(
            ...
            function (data) {
                ...
                $.each(data, function (i, e) {// 循环创建手风琴的项
                    ...
                    $.parser.parse();
                    $.get("/index/getTwoMenu?id=" + e.id , function (data) {// 循环创建树的项
                        $("#tree" + e.id).tree({
                            data: data,
                            lines: false,
                            animate: true,
                            onBeforeExpand: function (node, param) {
                                $("#tree" + e.id).tree('options').url = "/index/getTwoMenu?id=" + node.id;
                            },
